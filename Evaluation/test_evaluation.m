...
names = {'U2Fusion'};  % 方法名，用于Excel文件命名
method_name = cellstr(names(1));
row = 'S';  % 数据生成在Excel中的哪一列
row_name = strrep(row_name1, 'row', row);
row_data = strrep(row_data1, 'row', row);
fileFolder=fullfile('F:\paper_workspace\test_result\RoadScene\dataset\ir'); % Folder where infrared images or visible images are located
dirOutput=dir(fullfile(fileFolder,'*.bmp')); % the suffix name of the source and fused images
fileNames = {dirOutput.name};
[m, num] = size(fileNames);
ir_dir = 'F:\paper_workspace\test_result\RoadScene\dataset\ir'; % Folder where infrared images are located
vi_dir = 'F:\paper_workspace\test_result\RoadScene\dataset\vi'; % Folder where visible images are located
Fused_dir = 'F:\paper_workspace\test_result\RoadScene\U2Fusion'; % Folder where fused images are located
...
file_name = 'F:\paper_workspace\test_result\evaluation\road\metric.xls' % File name for writing metrics to excel
if easy ==1
